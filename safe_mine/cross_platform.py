...
def download_xmrig_windows() -> str | None:
    ...
    url = f"https://github.com/xmrig/xmrig/releases/download/v{cfg['xmrig_version']}/xmrig-{cfg['xmrig_version']}-msvc-win64.zip"
...
def calculate_threads() -> int:
    try:
        ...
        total = os.cpu_count()
    total = total or 1
    threads = max(1, int(total * cfg["cpu_usage_percent"]))
...
def start_miner(xmrig_path):
    ...
    # ================= WINDOWS (FIXED) =================
        ...
        allowed = max(1, int(total * cfg["cpu_usage_percent"]))
    ...
    while True:
        ...
        if temp is None:
            if not warned_no_temp:
                ...
                warned_no_temp = True
            time.sleep(cfg["temp_check_interval"])
            continue
        ...
        try:
            if temp >= cfg:
                log.critical("Emergency temperature reached (>= %s°C). Killing miner.", cfg['emergency_kill_temp_c'])
                shutdown()
            elif temp >= cfg['pause_temp_c'] and not suspended:
            ...
            elif suspended and temp <= cfg['resume_temp_c']:
                if ps_proc:
                    try:
                        ps_proc.resume()
                        suspended = False
